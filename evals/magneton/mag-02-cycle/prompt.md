...
runs: 5
---
Phase 3 — Structural verification. Spawn a sub-agent with the full authored plan below to validate contract completeness, stable ids, dependency-graph integrity, mode rules, and file-disjoint waves. It must not read repository anchors. Relay its output verbatim with no additions: one line per step (`OK` or `structural error — <specific violation>`), followed by a final `VALID` or `INVALID`.
