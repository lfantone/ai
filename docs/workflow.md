...
```
 /plan-orchestrator ──► /implement-orchestrator ──► /verify-orchestrator
        │                        │                          │
        └────────── all write to <cache>/plan-<ticket>.md ─────────┘
...
Findings are executable, too: `/implement-orchestrator <PR index>` runs in **review
mode** — it converts the review's findings into a fix plan (anchors + suggestions are
already step-shaped) and applies your selection. Push, re-review, threads auto-resolve.
