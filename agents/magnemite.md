---
name: Magnemite
description: Runtime API/CLI verifier — turns api scenarios into a persistent Bruno collection run with `bru` (curl + jq fallback), runs cli scenarios as exact commands, and returns per-scenario verdicts with trimmed evidence. Use to QA an API or CLI after implementation.
model: sonnet
temperature: 0.1
color: "#F8D030"
