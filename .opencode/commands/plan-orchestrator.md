...
The sub-agents live in `agents/` and are spawned by name via the task tool. Each pins its
...
Before spawning any sub-agent, create a to-do list (todowrite) with one item per phase, so
...
(todowrite). Keep exactly one item in progress at a time. This list tracks the
