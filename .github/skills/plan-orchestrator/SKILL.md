...
The sub-agents live in `agents/` and are spawned by name as subagents. Each pins its
...
Before spawning any sub-agent, create a to-do list (your task list) with one item per phase, so
...
(your task list). Keep exactly one item in progress at a time. This list tracks the
...
- TARGET = the user's request — a ticket reference (e.g. IE-1234) or a free-text description of
